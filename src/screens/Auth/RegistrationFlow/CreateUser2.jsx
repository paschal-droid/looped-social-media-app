...
import { createOTPRequest, firstTimeConfirmOTP, loginWithOTP } from '../../../api/authRequests';
...
const CreateUser2 = ({navigation, route}) => {
  // ! HOOKS
  ...
  const handleOTPVerification = async () => {
    ...
    const user = await firstTimeConfirmOTP(confirm, otp)
    if (user.status && user.isRegistered) {
        updateDeviceToken()
        navigation.dispatch(
          CommonActions.reset({
            index: 0,
