...
const VerifyUser1 = ({navigation}) => {
  ...
  const handleUserAuthentication = async () => {
    ...
    if(confirmation.confirm && confirmation.verificationId){
      setError('')
      navigation.navigate(Routes.LoginFlow2, {confirm: confirmation, readyNumber: readyNumber})
    }
    ...
    <SafeAreaView style={[{ backgroundColor: theme.background}, globalStyles.spacePadding, globalStyles.appScreen, styles.verificationContainer]}>
      ...
      <ScrollView contentContainerStyle={{ flexGrow: 1 }} showsVerticalScrollIndicator={false}>
        <View style={styles.countrySection}>
          ...
          <HeaderText first="What's your" second="phone Number?" subTextfirst={'Connect With Friends and Join in on the fun!'} />
          ...
          <View style={styles.authActions}>
            ...
            <Pressable style={{alignItems: 'center'}} onPress={() => navigation.navigate(Routes.RegistrationFlow)}>
              <Text style={[styles.authSwitchText, {color: theme.primaryColor}]}>New User? Sign Up Here</Text>
            </Pressable>
